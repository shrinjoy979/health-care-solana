...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => setActiveStep(s => (s + 1) % 5), 2500);
      ...
      <nav style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 48px", borderBottom: "0.5px solid rgba(29,158,117,0.2)", position: "sticky", top: 0, background: "rgba(10,15,13,0.92)", backdropFilter: "blur(12px)", zIndex: 100 }}>
        ...
        <div style={{ display: "flex", gap: 32, fontSize: 13, color: "#8aab9a" }}>
          {["Protocol", "How it works", "Tokenomics", "Docs"].map(n => (
            <span key={n} style={{ cursor: "pointer", transition: "color 0.2s" }} onMouseEnter={e => (e.target as HTMLElement).style.color = "#9FE1CB"} onMouseLeave={e => (e.target as HTMLElement).style.color = "#8aab9a"}>{n}</span>
          ))}
        </div>
        <button style={{ background: "#1D9E75", color: "#0a0f0d", border: "none", borderRadius: 6, padding: "9px 20px", fontFamily: "'Outfit',sans-serif", fontWeight: 500, fontSize: 13, cursor: "pointer" }}>
          Connect wallet
        </button>
      </nav>
      ...
      <section style={{ padding: "100px 48px 80px", maxWidth: 1200, margin: "0 auto", position: "relative" }}>
        ...
        <div style={{ opacity: visible ? 1 : 0, animation: visible ? "fadeUp 0.8s ease forwards" : "none" }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(48px,7vw,88px)", fontWeight: 900, lineHeight: 1.0, marginBottom: 28, maxWidth: 780 }}>
            Healthcare where<br />
            <span style={{ color: "#1D9E75", fontStyle: "italic" }}>outcomes</span> pay,<br />
            not procedures.
          </h1>
          ...
          <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
            <button style={{ background: "#1D9E75", color: "#0a0f0d", border: "none", borderRadius: 8, padding: "14px 32px", fontFamily: "'Outfit',sans-serif", fontWeight: 500, fontSize: 15, cursor: "pointer", letterSpacing: "0.01em" }}>
              Launch app →
            </button>
            <button style={{ background: "transparent", color: "#9FE1CB", border: "0.5px solid rgba(29,158,117,0.4)", borderRadius: 8, padding: "14px 28px", fontFamily: "'Outfit',sans-serif", fontWeight: 400, fontSize: 15, cursor: "pointer" }}>
              Read the essay
            </button>
          </div>
        </div>
      ...
      <section style={{ padding: "100px 48px", maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center" }}>
          <div>
            ...
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 42, fontWeight: 700, lineHeight: 1.2, marginBottom: 20 }}>
              Skin in the game.<br />For both sides.
            </h2>
            <p style={{ fontSize: 15, color: "#8aab9a", lineHeight: 1.8, fontWeight: 300 }}>
              Traditional healthcare rewards volume. We reward outcomes. Every treatment recommendation is a bet — practitioners stake their tokens on your recovery.
            </p>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {steps.map((s, i) => (
              <div key={i} onClick={() => setActiveStep(i)} style={{ display: "flex", gap: 16, padding: "18px 20px", borderRadius: 12, border: `0.5px solid ${activeStep === i ? "rgba(29,158,117,0.5)" : "rgba(29,158,117,0.1)"}`, background: activeStep === i ? "rgba(29,158,117,0.06)" : "transparent", cursor: "pointer", transition: "all 0.3s ease" }}>
                ...
                <div>
                  <div style={{ fontSize: 14, fontWeight: 500, color: activeStep === i ? "#e8f0eb" : "#8aab9a", marginBottom: 4, transition: "color 0.3s" }}>{s.title}</div>
